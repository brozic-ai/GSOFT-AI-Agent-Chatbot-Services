...
class ChatRepository:
    ...
    def update_conversation_title(self, conversation_id: Any, title: str, source: str = "llm") -> None:
        ...
        try:
            ...
            if conv:
                ...
                db.commit()
                logger.info("[OK] Updated title for Conversation ID=%s", conv_id)
        except Exception as ex:
            db.rollback()
    ...
    def delete_conversation(self, conversation_id: Any, user_id: str) -> bool:
        ...
        try:
            ...
            db.delete(conv)
            db.commit()
            logger.info("[OK] Deleted Conversation ID=%s for user_id='%s'", conv_id, user_id)
            return True
        except Exception as ex:
            db.rollback()
    ...
    def save_message(self, conversation_id: Any, role: str, content: str) -> int:
        ...
        try:
            ...
            db.commit()
            db.refresh(msg)
            logger.debug("[OK] Saved message ID=%s (role='%s') for conversation_id=%s", msg.id, role, conv_id)
            return msg.id
        except Exception as ex:
            db.rollback()
    ...
    def get_message_count(self, conversation_id: Any) -> int:
        ...
        try:
            conv_id = int(conversation_id)
            return db.query(ChatMessage).filter(ChatMessage.conversation_id == conv_id).count()
        finally:
            db.close()
